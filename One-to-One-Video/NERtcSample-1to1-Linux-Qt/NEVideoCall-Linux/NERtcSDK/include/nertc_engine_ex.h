...
public:
    ...
    */
     ...
     * @param[in] pitch 按半音音阶调整本地播放音乐的音调，默认值为0，即不调整音调。取值范围为 [-12,12]。
     * @return
     * - 0: 方法调用成功；
     * - 30003：参数错误，pitch超出范围。
     * - 30005：当前状态不支持的操作，可能找不到对应的伴音或SDK 未初始化。
     * - 30101：通话未开始。(仅pc ?)
     * - 其他: 方法调用失败。
     * @endif
     */
     ...
     * 发起伴音后可调解。伴音结束后再发起需要重新设置。
     * 音调pitch取值范围为 [-12,12]，每相邻两个值的音高距离相差半音。取值的绝对值越大，音调升高或降低得越多。
     * @param[out] pitch 按半音音阶调整本地播放音乐的音调，默认值为0，即不调整音调。取值范围为 [-12,12]。
     * @return
     * - 0: 方法调用成功；
     * - 30005：当前状态不支持的操作，可能找不到对应的伴音或SDK 未初始化。
     * - 30101：通话未开始。(仅pc ?)
     ...
     * - 0: 方法调用成功；
     * - 30003：参数错误，pitch超出范围。
     * - 30005：当前状态不支持的操作，可能找不到对应的音效或SDK 未初始化。
     * - 30101：通话未开始。(仅pc ?)
     * - 其他: 方法调用失败。
     * @endif
     */
    ...
    */
     ...
     * @if Chinese
     * 注册视频编码qos观测器。
     * @note 需要在初始化SDK后设置，释放SDK 后失效。
     * @since V4.6.25
     * @param[in] observer  接口对象实例。如果传入 NULL，则取消注册。
     * @return
     * - 0: 方法调用成功；
     * - 其他: 方法调用失败。
    * @endif
    */
     ...
     * @if Chinese
     * 推送外部视频编码帧。
     * <br>该方法主动将视频帧数据用 NERtcVideoEncodedFrame 类封装后传递给 SDK。 请确保在你调用本方法前已调用 setExternalVideoSource，并将参数设为 true，否则调用本方法后会一直报错。
     * <br>请确保在你调用本方法前已调用 enableDualStreamMode，并将参数设为 fasle，否则对方可能收不到视频。
     * @note 该方法设置内部引擎为启用状态，在 \ref IRtcEngine::leaveChannel "leaveChannel" 后不再有效。
     * @since V4.6.25
     * @param[in] encoded_frame         视频帧数据。
     * @return
     * - 0: 方法调用成功；
     * - 其他: 方法调用失败。
    * @endif
    ...
    */
     ...
     * @if Chinese
     * 推送外部音频编码数据输入。
     * <br>将外部音频编码数据帧推送给内部引擎。 通过 setExternalAudioSource 启用外部音频数据输入功能成功后，可以使用 pushExternalAudioEncodedFrame 接口发送音频编码后数据。
     * @note 
     * - 该方法需要在加入房间后调用。
     * - 该方法在音频输入设备关闭后不再生效。例如关闭本地音频、通话结束，该设置不再生效。
     * @since V4.6.25
     * @param[in] encoded_frame 外部输入编码数据帧。
     * @return
     * - 0: 方法调用成功；
     * - 其他: 方法调用失败。
    * @endif
    */
     ...
     * @if Chinese
     * 推送外部音频编码数据输入。
     * <br>将外部音频编码数据帧推送给内部引擎。 通过 setExternalSubStreamAudioSource 启用外部音频数据输入功能成功后，可以使用 pushExternalSubStreamAudioEncodedFrame 接口发送音频编码后数据。
     * @note 
     * - 该方法需要在加入房间后调用。
     * - 该方法在音频输入设备关闭后不再生效。例如关闭本地音频、通话结束，该设置不再生效。
     * @since V4.6.25
...
（停止发送音频）的回调。
	...
	/**
     * @if Chinese
    更新权限密钥。需要在通话中设置，异步结果见 \ref onUpdatePermissionKey 。
